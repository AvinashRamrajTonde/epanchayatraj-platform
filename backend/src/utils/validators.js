...
export const createMemberSchemaExtended = Joi.object({
  name: Joi.string().min(2).max(100).required(),
  designation: Joi.string().min(2).max(100).required(),
  type: Joi.string().valid('sarpanch', 'upsarpanch', 'gramsevak', 'grampanchayat_adhikari', 'leader', 'member', 'staff', 'computer_operator', 'pump_operator', 'safai_kamgar', 'peon', 'other_staff', 'rojgar_sevak', 'lipik').optional(),
  phone: Joi.string().max(15).allow('', null).optional(),
  email: Joi.string().email().allow('', null).optional(),
  photoUrl: imageUrl().allow('', null).optional(),
